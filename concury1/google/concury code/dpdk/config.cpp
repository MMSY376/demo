...
parse_arg_rx(const char *arg) {
  ...
  while ((p = strchr(p0, '(')) != NULL) {
    ...
    const size_t n_queues = RTE_MIN(lp->io.rx.n_nic_queues, RTE_DIM(lp->io.rx.nic_queues));
...
parse_arg_w(const char *arg) {
  ...
  // It is correct to comment out this if statement. Because we modified the RSS part.
  // The assumption is: the least significant byte of src ip address is randomly distributed within 0~255
...
static int
parse_arg_ratio(const char *arg) {
  app.ratio = atoi(arg);
  return 0;
}
...
static int
parse_arg_nk(const char *arg) {
  uint64_t x;
  
  extern uint STO_NUM, CONN_NUM;                // simulate control plane
  if (strnlen(arg, APP_ARG_NUMERICAL_SIZE_CHARS + 1) == APP_ARG_NUMERICAL_SIZE_CHARS + 1) {
    return -1;
  }
...
//          }
//        }
        if (!strcmp(lgopts[option_index].name, "Nk")) {
          arg_nk = 1;
          ret = parse_arg_nk(optarg);
          if (ret) {
            printf("Incorrect value for --Nk argument (%d)\n", ret);
            return -1;
          }
        }
    
        if (!strcmp(lgopts[option_index].name, "ratio")) {
          arg_nk = 1;
          ret = parse_arg_ratio(optarg);
          if (ret) {
            printf("Incorrect value for --ratio argument (%d)\n", ret);
            return -1;
          }
        }
        break;
  ...
  if ((arg_rx == 0) || (arg_tx == 0) || (arg_w == 0) || (arg_nk == 0)) {
...
int app_get_lcore_for_nic_rx(uint16_t port, uint8_t queue, uint32_t *lcore_out) {
  uint32_t lcore;
  ...
  for (lcore = 0; lcore < APP_MAX_LCORES; lcore++) {
    ...
    const size_t n_queues = RTE_MIN(lp->rx.n_nic_queues, RTE_DIM(lp->rx.nic_queues));
    for (i = 0; i < n_queues; i++) {
