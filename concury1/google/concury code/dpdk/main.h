...
//#error "APP_MAX_NIC_TX_PORTS_PER_IO_LCORE too big"     // TODO
#endif
...
struct app_mbuf_array {
  ...
  uint64_t hashes[APP_MBUF_ARRAY_SIZE];
  uint32_t n_mbufs;
};
...
struct app_lcore_params_io {
  ...
  struct {
    ...
    /* Rings, one for each worker. writer is the io thread. reader is the worker */
    struct rte_ring *rings[APP_MAX_WORKER_LCORES];
    uint32_t n_rings;
    
    /* buffer from nic queue to io thread. data here may be from different ports and queues */
    struct app_mbuf_array mbuf_in;
    /* buffer from io thread to worker. the reason for this is to even the loads between workers */
...
struct app_params {
  ...
  /* for load balancing
   * The position of the 1-byte field within the input packet used by the I/O RX lcores to identify the worker lcore for the current packet.
   * */
  uint8_t pos_lb;
  uint32_t ratio;
} __rte_cache_aligned;
