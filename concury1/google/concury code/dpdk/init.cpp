...
static void app_init_mbuf_pools(void) {  // per socket
  ...
  for (socket = 0; socket < APP_MAX_SOCKETS; socket++) {
    ...
    app.pools[socket] = rte_pktmbuf_pool_create(
      name, APP_DEFAULT_MEMPOOL_BUFFERS, APP_DEFAULT_MEMPOOL_CACHE_SIZE, 0, APP_DEFAULT_MBUF_DATA_SIZE, socket);
  ...
  for (lcore = 0; lcore < APP_MAX_LCORES; lcore++) {
    ...
    app.lcore_params[lcore].pool = app.pools[socket]; // TODO why assign each socket has a single pool? what's the relation between pool and mbuf?
  }
}

static void app_init_lpm_tables(void) {  // per socket
  unsigned socket, lcore;
  ...
  // assign the created lpm table to all worker lcores in this socket
  for (lcore = 0; lcore < APP_MAX_LCORES; lcore++) {
    if (app.lcore_params[lcore].type != e_APP_LCORE_WORKER) {
      continue;
    }
...
static void app_init_rings_rx(void) {
  ...
  for (lcore = 0; lcore < APP_MAX_LCORES; lcore++) {
    struct app_lcore_params_io *lp_io = &app.lcore_params[lcore].io;  // io maybe invalid for worker type
    ...
    // for each io thread, find all workers on the same socket and assign a ring for each of them
    ...
    for (lcore_worker = 0; lcore_worker < APP_MAX_LCORES; lcore_worker++) {
      ...
      snprintf(name, sizeof(name), "app_ring_rx_s%u_io%u_w%u", socket_io, lcore, lcore_worker);
...
static void app_init_nics(void) {
  ...
  for (port = 0; port < APP_MAX_NIC_PORTS; port++) {
    ...
    n_rx_queues = app_get_nic_rx_queues_per_port(port);     // actually no doc is about the packet distribution of packets into queues. so always use one queue at one port. It is specified from the command line
    n_tx_queues = app.nic_tx_port_mask[port];  // 0 or 1. always single queue for tx
    
    if ((n_rx_queues == 0) && (n_tx_queues == 0)) {
      continue;
    }
    
    // each port on each nic. ports appear as devs
    ...
    ret = rte_eth_dev_configure(port, (uint8_t) n_rx_queues, (uint8_t) n_tx_queues, &local_port_conf);    // RSS config is passed here
    if (ret < 0) {
    ...
    ret = rte_eth_dev_adjust_nb_rx_tx_desc(port, &nic_rx_ring_size, &nic_tx_ring_size);
    if (ret < 0) {
    ...
    /* Init RX queue for every port. the info is ultimately from the command line*/
    ...
    for (queue = 0; queue < APP_MAX_RX_QUEUES_PER_NIC_PORT; queue++) {
      ...
      ret = rte_eth_rx_queue_setup(port, queue, (uint16_t) app.nic_rx_ring_size, socket, &rxq_conf, pool);
      if (ret < 0) {
    ...
    if (app.nic_tx_port_mask[port] == 1) {
      ...
      ret = rte_eth_tx_queue_setup(port, 0, (uint16_t) app.nic_tx_ring_size, socket, &txq_conf);
      if (ret < 0) {
