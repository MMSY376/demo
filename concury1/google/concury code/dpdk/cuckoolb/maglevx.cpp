...
// Data & Control plane
MySimpleArray<ControlPlaneCuckooMap<uint64_t, uint16_t, uint8_t, false>> connTrackingTables; // 3-tuple -> DIPInd requires initialization
...
extern Hasher32<Tuple3> hasher[2];
...
void printMemoryUsage() {
  ...
  for (int i = 0; i < VIP_NUM; ++i)
    count += connTrackingTables[i].EntryCount();
...
void serve() {
  ...
  while (round < 5) {
    ...
    if (connTrackingTables[vipInd].Find(hash, htInd)) {
      ...
      connTrackingTables[vipInd].Insert(hash, htInd);
      uint16_t tmp;
      assert(connTrackingTables[vipInd].Find(hash, tmp) && tmp == htInd);
    }
    
    DIP &dip = dipPools[vipInd][ht[vipInd][htInd]];
    assert(((dip.addr.addr ^ (0x0a000000 + (vipInd << 8))) < dipPools[vipInd].capacity &&
            dip.addr.port - vip.port >= 0 && dip.addr.port - vip.port < dipPools[vipInd].capacity));
...
//    for (int i = 0; i < VIP_NUM; ++i)
//      connTrackingTables[i].Clear(CONN_NUM / VIP_NUM);
...
//    cout << "insert preparation: " << vip << " " << tuple << " " << hash << " " << endl;
    const uint64_t *res = connTrackingTables[vipInd].Insert(hash, dipInd);
//    cout << "insert result: " << (res == &hash) << " " << endl;

//      #ifndef NDEBUG
//      uint16_t tmp;
//      assert(connTrackingTables[vipInd].Find(hash, tmp));
...
//      #endif
  ...
  for (int i = 0; i < limit; i++) {
    ...
    uint16_t tmp;
    connTrackingTables[vipInd].Find(hash, tmp);
    if (tmp != dipInd) collisionCnt++;
//      #ifndef NDEBUG
//      uint16_t tmp;
//      assert(connTrackingTables[vipInd].Find(hash, tmp));
//      if (tmp != dipInd) Counter::count("Maglevx", "collision");
//      #endif
  ...
  cout << "collision: " << human(collisionCnt) << endl;
//  }
...
void checkCollision() {
  ...
  for (int i = 0; i < STO_NUM; i++) {
    ...
    // Step 4: check
    uint16_t tmp;
    if (connTrackingTables[vipInd].Find(hash, tmp)) {
      collision++;
    }
  }
...
void updateDataPlane(bool mute = false) {
  ...
  for (uint16_t vipInd = 0; vipInd < VIP_NUM; ++vipInd) {
    ...
    for (int dipInd = 0; dipInd < dipCount; ++dipInd) {
      ...
      int entries = (allocatedWeight + w) * HT_SIZE / weightSum - allocatedEntries + 0.5;
    ...
    connTrackingTables[vipInd].Compose(migration);
...
void initControlPlaneAndDataPlane() {
  ...
  connTrackingTables.resize(VIP_NUM);
  
  for (int i = 0; i < VIP_NUM; ++i) {
    ht[i].resize(HT_SIZE);
    newHt[i].resize(HT_SIZE);
    connTrackingTables[i].Clear(CONN_NUM / VIP_NUM * 1.5);
  }
...
//      DIP &dip = dipPools[vipInd][ht[vipInd][htInd]];
//      assert(((dip.addr.addr ^ (0x0a000000 + (vipInd << 8))) < dipPools[vipInd].capacity &&
//              dip.addr.port - tuple.dst.port >= 0 && dip.addr.port - tuple.dst.port < dipPools[vipInd].capacity));
//
//      stupid += dip.addr.addr;   //prevent optimize
//    }
...
//int maglevx_main(int argc, char **argv) {
//  init();
////  cout << "checkCollision" << endl;
////  checkCollision();
//  simulateConnectionAdd();
