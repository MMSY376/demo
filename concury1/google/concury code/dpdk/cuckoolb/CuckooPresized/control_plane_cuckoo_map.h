...
template<class Key, class Value, class Match, int kCandidateBuckets, int kSlotsPerBucket>
class DataPlaneCuckooMap;

template<class K, bool allowGateway, class Match>
class BloomFilterControlPlane;

template<class Key, class Value, class Match = uint8_t, bool willExport = true, int kCandidateBuckets = 2, int kSlotsPerBucket = 4>
class ControlPlaneCuckooMap {
  friend class DataPlaneCuckooMap<Key, Value, Match, kCandidateBuckets, kSlotsPerBucket>;
  
  template<class K, class V, class M, bool E, int B, int S> friend
  class ControlPlaneCuckooMap;
  
  template<class K, bool G, class M, bool T> friend
...
public:
  ...
  explicit operator ControlPlaneCuckooMap<Key, Value, Match, false, kCandidateBuckets, kSlotsPerBucket>() const {
    ControlPlaneCuckooMap<Key, Value, Match, false, kCandidateBuckets, kSlotsPerBucket> other;
...
//    }
  ...
  const Key *Insert(const Key &k, const Value &v) {
...
//          if (getDigest(key) == getDigest(k)) {
//            Counter::count("Cuckoo", "cuckoo collision");
...
//    }
    ...
    if (CuckooInsert(k, v)) {
      ...
      Counter::count("Cuckoo", "cuckoo insert fail");
      entryCount--;
      return nullptr;
    }
  }
  ...
  inline bool Remove(const Key &k, bool delCollision = true) {
    for (int i = 0; i < kCandidateBuckets; ++i) {
      ...
      if (RemoveInBucket(k, bucket)) {
        entryCount--;
        if (willExport && delCollision) deleteCollision(k);
        return true;
      }
    }
  ...
  inline vector<const Key *> FindAllCollisions(const Key &k) {
...
private:
  ...
  struct Bucket : public base_class<willExport> {
  public:
  ...
  template<bool E>
  inline typename std::enable_if<E, void>::type setDigest(Bucket &bdst, int slot, Match digest) {
    bdst.digests[slot] = digest;
  }
  ...
  inline typename std::enable_if<E, void>::type setDigest(Bucket &bdst, int sdst, const Bucket &bsrc, int ssrc) {
    bdst.digests[sdst] = bsrc.digests[ssrc == -1 ? sdst : ssrc];
  }
...
//      collisionSets[bucket].push_back(k);
  ...
  inline void InsertInternal(const Key &k, const Value &v, uint32_t b, int slot) {
    ...
    setDigest<willExport>(bptr, slot, getDigest(k));
    
    bptr.occupiedMask |= 1U << slot;
    
    if (willExport) insertCollision(k);
    
//    cout << hex << k << " inserted to: " << b << " " << slot << ", current mask: " << int(bptr.occupiedMask) << endl;
  }
...
//    cout << endl;
  ...
  inline void CopyItem(uint32_t src_bucket, int src_slot, uint32_t dst_bucket, int dst_slot) {
    ...
    setDigest<willExport>(dst_ref, dst_slot, src_ref, src_slot);
  }
  ...
  MySimpleArray<Bucket> buckets_;
  
  CuckooPathQueue cpq_;
  CuckooPathEntry visited_[kVisitedListSize];
//  MySimpleArray<vector<Key>> collisionSets;
};

template<class Key, class Value, class Match = uint16_t, int kCandidateBuckets = 2, int kSlotsPerBucket = 4>
class DataPlaneCuckooMap {
public:
  explicit DataPlaneCuckooMap(
    const ControlPlaneCuckooMap<Key, Value, Match, true, kCandidateBuckets, kSlotsPerBucket> &controlPlane
    ...
    for (int ii = 0; ii < controlPlane.buckets_.capacity; ++ii) {
      auto &b = controlPlane.buckets_[ii];
      Bucket bucket;
...
private:
  ...
  bool FindInBucket(Match digest, uint32_t b, Value &out) const {
    const Bucket &bref = buckets_[b];
    for (int i = 0; i < kSlotsPerBucket; i++) {
      if ((bref.occupiedMask & (1ULL << i)) && bref.keyDigests[i] == digest) {
        out = bref.values[i];
        return true;
      }
    }
